...
def map(reference, threads, plus_tag, minus_tag, folder):
    ...
    for each_fq in file_name:
        ...
        map_command = ('''bwa mem -t %s -M -R \'@RG\\tID:%s\\tSM:%s\\tLB:WES\\tPL:Illumina\' %s %s %s > %s/%s.pre_del_sort.bam''' %(threads, sample_name, sample_name, reference, plus_fq, minus_fq, folder,sample_name))
