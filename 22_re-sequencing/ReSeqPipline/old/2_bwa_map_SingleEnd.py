...
def map(reference, threads, plus_tag, folder):
    ...
    for each_fq in file_name:
        ...
        map_command = ('''bwa mem -t %s -M -R \'@RG\\tID:%s\\tSM:%s\\tLB:WES\\tPL:Illumina\' %s %s > %s/%s.pre_del_sort.bam''' %(threads, sample_name, sample_name, reference, plus_fq, folder,sample_name))
        print("Map to Reference with bwa command: " + map_command)
        os.system(map_command)
        #运行samtools删除多重击中文件
        samtools_command1 = ('''samtools view -q 1 -b %s/%s.pre_del_sort.bam > %s/%s.pre_sort.bam''' %(folder, sample_name, folder, sample_name))
        print("Delete Multiple Hits with samtools command: " + samtools_command1)
...
def main():
    ...
    map(reference, threads, plus_tag, folder) #主程序   
