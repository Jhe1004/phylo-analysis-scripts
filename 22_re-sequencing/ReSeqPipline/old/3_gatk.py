...
def main_software(each_file_name_list,reference,folder):
    ...
    for each_file in each_file_name_list:
        each_sample_name = each_file.replace(".bam","")
        command = ('''./gatk HaplotypeCaller -ERC BP_RESOLUTION -R %s -I %s.bam -O %s.vcf''' %(reference, folder + os.sep + each_sample_name, each_sample_name))
        print("Run gatk with command " + command)
...
def main():
    ...
    additional.add_argument('-f', '--folder', action="store", metavar='\b', type=str,  default="current_location", help="存放bam文件的文件夹路径，默认为本脚本所在的文件夹")
